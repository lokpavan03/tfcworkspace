...
if [ ! -z "$Token" ]; then
  token=$Token
  echo "Token environment variable was found."
else
  echo "Token environment variable was not available."
  echo "Exiting."
  exit
fi
...
# Evaluate $TFC_URL environment variable if it exists
if [ ! -z "$TFC_URL" ]; then
  address=$TFC_URL
  echo "Using address, ${address}"
else
  address="app.terraform.io"
  echo "TFC_URL environment variable was not set."
fi
...
# If first argument is null "", then load code from config directory
if [ ! -z $1 ]; then
  git_url=$1
...
# Set workspace if provided as the second argument as null
if [ ! -z "$2" ]; then
  workspace=$2
...
if [ -z "$workspace_id" ]; then
  ...
  workspace_result=$(curl -s --header "Authorization: Bearer $Token" --header "Content-Type: application/vnd.api+json" --request POST --data @workspace.json "https://${address}/api/v2/organizations/${organization}/workspaces")
...
configuration_version_result=$(curl -s --header "Authorization: Bearer $Token" --header "Content-Type: application/vnd.api+json" --data @configversion.json "https://${address}/api/v2/workspaces/${workspace_id}/configuration-versions")
...
if [[ "$save_plan" == "true" ]]; then
  ...
  plan_result=$(curl -s --header "Authorization: Bearer $Token" --header "Content-Type: application/vnd.api+json" https://${address}/api/v2/runs/${run_id}?include=plan)
...
if [[ "$applied" == "true" ]]; then
  ...
  check_result=$(curl -s --header "Authorization: Bearer $Token" --header "Content-Type: application/vnd.api+json" https://${address}/api/v2/runs/${run_id}?include=apply)
  ...
  while [ $continue -ne 0 ]; do
    ...
    check_result=$(curl -s --header "Authorization: Bearer $Token" --header "Content-Type: application/vnd.api+json" https://${address}/api/v2/applies/${apply_id})
  ...
  state_file_url_result=$(curl -s --header "Authorization: Bearer $Token" "https://${address}/api/v2/state-versions/${state_id}?include=outputs")
